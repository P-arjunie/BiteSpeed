...
const MenuManagement = () => {
  ...
  const fetchMenuItems = async (id) => {
    try {
      const res = await fetch(`https://localhost:30082/api/menu/${id}`);
      const data = await res.json();
      setMenuItems(data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditing
        ? `https://localhost:30082/api/menu/${editingItemId}`
        : "https://localhost:30082/api/menu";
  ...
  const toggleAvailability = async (item) => {
    const token = localStorage.getItem("restaurantToken");
    try {
      const res = await fetch(`https://localhost:30082/api/menu/${item._id}`, {
        method: "PUT",
        headers: {
        ...
        <div className="relative z-10 p-6">
          ...
          <div className="bg-white shadow rounded">
            <div className="p-4 border-b">
              <h2 className="text-lg font-semibold">📋 View All Items</h2>
            </div>
                      ...
                      <tr key={item._id} className="border-b hover:bg-yellow-50">
                        ...
                        <td className="p-2 flex items-center gap-2">
                          <button
                            ...
                            className="p-1 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 text-lg"
                          >
                            <FaEdit />
                          </button>
                          <button
                            onClick={() => handleDelete(item._id)}
                            className="p-1 rounded-full bg-red-100 text-red-600 hover:bg-red-200 text-lg"
                          >
                            <FaTrash />
                          </button>
                        </td>
